...
typedef enum TMsgID {
    ...
    Msg_GetPowerTimerCfg       =101,
    Msg_SetPowerTimerCfg       =102,
    
    Msg_UpgradeBin             =103, //httpPost方式升级固件
    Msg_GetDoorCfg             =104, //获取门配置
    Msg_SetDoorCfg             =105, //设置门配置
    Msg_DoorControl            =106, //门锁控制
    Msg_GetDoorStatus          =107, //获取门状态
...
typedef struct TFunctionMask {
    union {
        int Value;//all value
        struct {
            int IsOld:1;//旧新标志 =1 为旧的，=0为新的
            int IsVideo:1;
            int IsAudioMIC:1;//MIC
            int IsAudioHorn:1;//喇叭
            int IsPTZ:1;
            int IsLight:1;
            int IsPIR:1;
            int IsTimerPowerOnOff:1;//bit 7
            int IsDoorControl:1;//bit 8
            //...
        };
    };
}TFunctionMask;
...
//-----------------------------------------------------------------------------
typedef struct TPowerCfg {//sizeof 8
    u8 PowerOnActive;
    u8 PowerOnHour;
    u8 PowerOnMinute;
    u8 Reserved0;
    u16 PowerOffDelayMinute;//>0为多少分钟后自动关机 =0为不自动关机
    u8 Reserved1;
    u8 Reserved2;
}TPowerCfg;
//-----------------------------------------------------------------------------
typedef struct TDoorCfgPkt {//sizeof 240
#define MAXDOORCOUNT  10
    struct {
        u8 Active;//是否启用
        u8 Status;//门状态
        u8 Reserved0;
...
//-----------------------------------------------------------------------------
typedef struct TDoorControlPkt {//sizeof 4
    u8 Chl;
#define Cmd_Door_NULL  0
#define Cmd_Door_Open  1
#define Cmd_Door_Close 2
#define Cmd_Door_Stop  3
#define Cmd_Door_Lock  4
    u8 Cmd;
    u8 Reserved0;
    u8 Reserved1;
}TDoorControlPkt;
